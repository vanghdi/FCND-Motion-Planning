...
def valid_3d_actions(grid, current_node):
    """
    Returns a list of valid ThreeDAction given a voxel grid and current node.
    """
    ...
    if x + 1 > n or y - 1 < 0 or grid[x + 1, y - 1, z] == 1:
        valid_actions.remove(Action3D.SOUTH_WEST)
    if x + 1 > n or y + 1 > m or grid[x + 1, y + 1, z] == 1:
        valid_actions.remove(Action3D.SOUTH_EAST)
...
def a_star_3d(grid, h, start, goal):
    ...
    while not queue.empty():
        ...
        if current_node == goal:
            ...
            for a in valid_3d_actions(grid, current_node):
